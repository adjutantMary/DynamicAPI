...
class BonusRule(models.Model):
    ...
    condition_type = models.CharField(max_length=32, choices=RuleConditionType.choices)
    condition_value = models.JSONField(blank=True, null=True)
    operation_type = models.CharField(max_length=32, choices=RuleOperationType.choices)
...
class BonusLog(models.Model):
    ...
    def __str__(self):
        return f"[{self.created_at}] {self.rule_code}: +{self.bonus_delta} → {self.total_bonus}"
